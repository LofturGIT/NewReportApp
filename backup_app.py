...
def upload_files():
    if request.method == 'POST':
        # Check if user wants to reuse previous pending users data
        reuse_pending = 'reuse_pending' in request.form
        pending_users_path = None

        if reuse_pending and 'pending_users_path' in session:
            pending_users_path = session['pending_users_path']
        else:
            pending_users_file = request.files.get('pending_users')
            if not pending_users_file:
                return "Please upload a Pending Users file or select 'Reuse Previous Data'", 400
            pending_users_path = os.path.join(UPLOAD_FOLDER, pending_users_file.filename)
            pending_users_file.save(pending_users_path)
            session['pending_users_path'] = pending_users_path

        # Get the date filter for pending users
        invite_date_filter = request.form.get('invite_date')
        if invite_date_filter:
            invite_date_filter = datetime.strptime(invite_date_filter, '%Y-%m-%d')

        # Get all uploaded course status files
        course_status_files = request.files.getlist('course_status')
        if not course_status_files:
            return "Please upload at least one Course Status file!", 400
        ...
        # Process files
        report_path = process_files(pending_users_path, course_status_paths, invite_date_filter)
...
def process_files(pending_users_path, course_status_paths, invite_date_filter):
    ...
    if invite_date_filter:
        # Filter pending users by the invite date
        pending_users_df['Last invite sent at'] = pd.to_datetime(pending_users_df['Last invite sent at'], errors='coerce')
        pending_users_df = pending_users_df[pending_users_df['Last invite sent at'] > invite_date_filter]
    ...
    for course_status_path in course_status_paths:
        ...
        pending_users['Completed'] = pending_users['Last invite sent at'].apply(
            lambda x: f"Invite last sent: {x.strftime('%Y-%m-%d')}" if pd.notna(x) else "Unknown"
        )
